...
public class cEntity {
    ...
    /**
     * cEntity class
    A cEntity object represents an object in the world, it has a position and orientation.
    cEntity is an abstract class, and can not be instantiated directly, instead, all entities are implemented as subclasses.
    The cEntity class works as the common interface for the operations that all (most) entities support.
	
    All cEntity objects have an Entity Type so it can be determined what kind of entity it is efficiently.
    Entities also have a class inheritance awareness, they know their class name, their parent class' name and can decide if there is a class within their inheritance chain.
    Since these functions operate on strings, they are slightly slower than checking the entity type directly, on the other hand, they are more specific directly.
    To check if the entity is a spider, you need to call IsMob(), then cast the object to cMonster and finally compare cMonster:GetMonsterType() to mtSpider.
    GetClass(), on the other hand, returns "cSpider" directly.
	
    Note that you should not store a cEntity object between two hooks' calls, because Cuberite may despawn / remove that entity in between the calls.
    If you need to refer to an entity later, use its UniqueID and cWorld's entity manipulation functions DoWithEntityByID(), ForEachEntity() or ForEachEntityInChunk() to access the entity again.
     */
    ...
    class AddPosition extends TwoArgFunction {
        ...
        public LuaValue call(LuaValue luaEntity, LuaValue vector) {
        	entity.getPosition().add(Vector3d.toPosition(vector));
            return NIL;
        }
    }
    class AddSpeed extends TwoArgFunction {
         ...
         */
        public LuaValue call(LuaValue luaEntity, LuaValue vector) {
        	entity.getVelocity().add(Vector3d.toPosition(vector).toVector());
            return NIL;
        }
    }
    ...
    class GetLookVector extends TwoArgFunction {
         ...
         */
        public LuaValue call(LuaValue luaEntity, LuaValue value) {
            return Vector3f.from(entity.getPosition().getDirection());
        }
    }
    ...
    class GetParentClass extends TwoArgFunction {
        /**
         * string
         * Returns the name of the direct parent class or this entity
         */
        public LuaValue call(LuaValue luaEntity, LuaValue value) {
    ...
    class GetPosition extends TwoArgFunction {
         ...
         */
        public LuaValue call(LuaValue luaEntity, LuaValue value) {
            return Vector3d.from(entity.getPosition());
        }
    }
    ...
    class IsA extends TwoArgFunction {
         ...
         * ClassName	string
         * boolean
         * Returns true if the entity class s a descendant of the specified class name, or the specified class itself
         */
        public LuaValue call(LuaValue luaEntity, LuaValue value) {
    ...
    class MoveToWorld extends VarArgFunction {
         ...
         * World	cWorld
         * Position	Vector3d
         * ShouldSetPortalCooldown	boolean
         * ShouldSendRespawn	boolean
         * boolean
        	...
        	MinecraftServer.getInstanceManager().getInstances().forEach((instance) -> {
        		...
        		if (instance.getUniqueId() == uuid) {
        			...
        			instance.scheduleNextTick((deltaInstance) -> {
        				entity.teleport(Vector3d.toPosition(args[2]));
        			});
        		}
        	});
    ...
    class ScheduleMoveToWorld extends VarArgFunction {
         ...
         * ShouldSetPortalCooldown	boolean
         * ShouldSendRespawn	boolean
         * Schedules a MoveToWorld call to occur on the next Tick of the entity. If ShouldSetPortalCooldown is false (default), doesn't set any portal cooldown, if it is true, the default portal cooldown is applied to the entity. If ShouldSendRespawn is false, no respawn packet is sent, if it is true (default) then a respawn packet is sent to the client. OBSOLETE, use MoveToWorld instead.
         */
        public LuaValue call(LuaValue[] args) {
    ...
    class SetPosition extends TwoArgFunction {
         ...
         */
        public LuaValue call(LuaValue luaEntity, LuaValue vector) {
        	entity.getPosition().copyCoordinates(Vector3d.toPosition(vector));
            return NIL;
        }
    }
    ...
    class SetSpeed extends TwoArgFunction {
         ...
         */
        public LuaValue call(LuaValue luaEntity, LuaValue vector) {
        	entity.getVelocity().copyCoordinates(Vector3d.toVector(vector));
            return NIL;
        }
    }
    ...
    class TeleportToCoords extends TwoArgFunction {
        ...
        public LuaValue call(LuaValue luaEntity, LuaValue vector) {
            // TODO: Call hook
        	entity.teleport(Vector3d.toPosition(vector));
            return NIL;
        }
    }

...
public class cRoot {
    ...
    class DoWithPlayerByUUID extends VarArgFunction {
        ...
        public LuaValue call(LuaValue[] args) {
            Player player = MinecraftServer.getConnectionManager().getPlayer(UUID.fromString((args[1].get("ToShortString").call().tojstring())));
            if (player == null) {
            	return FALSE;
            } else {
    ...
    class ForEachWorld extends TwoArgFunction {
         ...
         * function Callback(cWorld). Returns false if a callback aborts, otherwise true.
         */
        public LuaValue call(LuaValue luaRoot, LuaValue value) {
    ...
    class GetPhysicalRAMUsage extends TwoArgFunction {
         ...
         */
        public LuaValue call(LuaValue luaRoot, LuaValue value) {
            return valueOf(((OperatingSystemMXBean) ManagementFactory.getOperatingSystemMXBean()).getTotalPhysicalMemorySize());
        }
    }

...
public class Vector3f {
	...
	public static LuaValue from(float X, float Y, float Z) {
		LuaValue vector = LuaValue.tableOf();
		vector.set("x", X);
		vector.set("y", Y);
		vector.set("z", Z);
	...
	static class multiply extends TwoArgFunction {
		public LuaValue call(LuaValue vector, LuaValue value) {
			if (value.isnumber()) {
				...
				return from(
				vector.get("x").mul(value.get("x").tofloat()),
				vector.get("y").mul(value.get("y").tofloat()),
				vector.get("z").mul(value.get("z").tofloat())
				);
			}
		}
	}
